...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        logging.error("Usage: python docstring.py <input_folder_directory> <output_folder_directory>")
        sys.exit(1)

    input_folder_directory = sys.argv[1]
    output_folder_directory = sys.argv[2]

    js_files_paths = Misc.get_js_files(input_folder_directory)
    ...
    for fileprocessinginfo in files:
        ...
        cleaned_output = Misc.output_cleaner(output_code_raw)
        Misc.write_output_in_structure(cleaned_output, fileprocessinginfo.filepath, input_folder_directory, output_folder_directory)
